...
def load_data(name):
  matrices = np.load(f"templates/{name}.npy")
...
def process():
  data = load_data("5x5_layer")
  test_data = load_data("5x5_layer_test")
  ...
  for i in range(len(data[0:200])):
    for j in range(10):
      X, y = data[i]
      X_t, y_t = test_data[i]
      X_test, y_test = generate_test_grid(X_t, y_t)

      random_state = (9 + 41*i - 2*j) % (2**16)
  ...
  save_dataframe("voronoi", "vor_5x5_layer_all", df)
