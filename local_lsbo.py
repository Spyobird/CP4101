...
# save a model
def save_lsbo_model(id, model, name):
  if not os.path.exists(f"models/local_lssm_lsbo/{id}"):
    print(f"[{id}] Creating directory for models")
    os.makedirs(f"models/local_lssm_lsbo/{id}", exist_ok=True)

  print(f"[{id}] Saving model")
  dump(model, f"models/local_lssm_lsbo/{id}/{name}.joblib")
...
def LSBo(X, y, distances, ne, ne_dist, ls):
  lsc = np.zeros(y.size)
...
def process(id, gamma, random_state=4101):
  ...
  for split in range(100):
    ...
    X_s, y_s = LSBo(X_train, y_train, distances, ne, ne_dist, ls)
    ...
    save_lsbo_model(id, clf, f"model_{id}_split{split:02d}")

    print(f"[{id}:{split}] Completed split")

  info_df = pd.DataFrame(info)
  info_df["mean"] = info_df.mean(numeric_only=True, axis=1)
  save_dataframe(id, "lssm_lsrb", "lsbo_info", "", info_df)
...
def main(args):
  if args.id:
    id = args.id
    print(f"LOCAL SET BORDER SELECTOR {id}")
